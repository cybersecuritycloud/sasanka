...
local function get_body_part()
	...
	local decompressed, eof, len_in, len_out = kong.ctx.shared.cap.inflate(ngx.arg[1])
...
local function get_deflated_body_part()
	...
	local compressed, eof, len_in, len_out = kong.ctx.shared.cap.deflate( kong.ctx.shared.cap.body_part , opt)
